 ...
 */
package org.kuali.kfs.module.ec.rest.application;
...
@ApplicationPath("/ec/api/v1")
public class EcApiApplication extends BaseApiApplication {

    public EcApiApplication() {
        super();
        if (SpringContext.getBean(ConfigurationService.class).getPropertyValueAsBoolean("apis.enabled")) {
            singletons.add(new BusinessObjectApiResource("ec"));
        }
    }
}