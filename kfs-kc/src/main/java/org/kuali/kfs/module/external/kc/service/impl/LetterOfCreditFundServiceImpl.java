...
public class LetterOfCreditFundServiceImpl implements ExternalizableBusinessObjectService {
    ...
    public Collection findMatching(Map fieldValues) {
        ...
        try {
            results = this.getWebService().findMatchingFund((String) fieldValues.get("fundCode"),
                    (String) fieldValues.get("description"));
        } catch (WebServiceException ex) {
            GlobalVariablesExtractHelper.insertError(KcConstants.WEBSERVICE_UNREACHABLE, getConfigurationService().getPropertyValueAsString(KFSConstants.KC_APPLICATION_URL_KEY));
        }

        List<LetterOfCreditFund> funds = new ArrayList<LetterOfCreditFund>();
        if (results != null) {
            for (LetterOfCreditFundDto dto : results) {
                funds.add(fundFromDTO(dto));
            }
        }
        return funds;
    }
