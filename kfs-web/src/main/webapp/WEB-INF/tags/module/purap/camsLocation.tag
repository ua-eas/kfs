...
<%@ attribute name="camsAssetLocationProperty" required="true"
...
<c:set var="refreshAssetLocationBuildingUrl"
       value="methodToCall.useOffCampusAssetLocationBuildingByItem.(((${ctr}))).((#${ctr2}#))"/>
<c:if test="${PurapConstants.CapitalAssetAvailability.ONCE eq availability}">
    <c:set var="deleteLocationUrl"
           value="methodToCall.deleteCapitalAssetLocationByDocument.(((${ctr}))).((#${ctr2}#))"/>
    <c:set var="refreshAssetLocationBuildingUrl"
           value="methodToCall.useOffCampusAssetLocationBuildingByDocument.(((${ctr}))).((#${ctr2}#))"/>
</c:if>
...
<table class="standard">
    <tr>
        <td colspan="4" class="subhead">
            <c:choose>
                <c:when test="${(fullEntryMode or amendmentEntry) and !poItemInactive and !(ctr2 eq 'new')}">
                    <div class="center">
                        <html:submit
    ...
    <tr>
        ...
        <td class="datacell" width="25%">
            ...
            <c:if test="${(fullEntryMode or amendmentEntry) && !poItemInactive}">
                <kul:lookup boClassName="org.kuali.kfs.vnd.businessobject.CampusParameter"
                            fieldConversions="campusCode:${camsAssetLocationProperty}.campusCode"/>
            </c:if>
        </td>
        <th width="25%" class="right">
        ...
        <td class="datacell" width="25%">
            <kul:htmlControlAttribute attributeEntry="${camsLocationAttributes.capitalAssetCityName}"
                                      ...
                                      readOnly="${!offCampus or !(fullEntryMode or amendmentEntry) or poItemInactive}"
