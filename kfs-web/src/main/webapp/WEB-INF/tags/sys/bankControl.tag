...
<%@ attribute name="objectProperty" required="true"
...
<%@ attribute name="readOnly" required="false"
              description="boolean indicating whether the document is readOnly. If not an additional check is made to verify the bank edit mode was exported." %>
...
<c:if test="${KualiForm.editingMode[KFSConstants.BANK_ENTRY_VIEWABLE_EDITING_MODE]}">
    ...
    <td class="${style}">
        ...
        <kul:htmlControlAttribute attributeEntry="${DataDictionary.Bank.attributes.bankCode}" property="${property}"
                                  readOnly="${readOnly}"
                                  onblur="loadBankInfo(document.forms['KualiForm'], '${property}', '${objectProperty}');"/>
        <c:if test="${not readOnly}">
            &nbsp;
            <kul:lookup boClassName="org.kuali.kfs.sys.businessobject.Bank"
                        fieldConversions="bankCode:${property},bankName:${objectProperty}.bankName"
                        lookupParameters="${lookupParameters}" readOnlyFields="${readOnlyFields}" autoSearch="true"/>
        </c:if>
