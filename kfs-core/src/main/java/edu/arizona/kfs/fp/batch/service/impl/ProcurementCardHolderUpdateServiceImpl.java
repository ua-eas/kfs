...
public class ProcurementCardHolderUpdateServiceImpl implements ProcurementCardHolderUpdateService {
    ...
    protected List<ProcurementCardHolderLoad> retrievePcardHolders() {
        ...
        List<ProcurementCardHolderLoad> loadedPcardHolders = (List<ProcurementCardHolderLoad>) businessObjectService.findMatchingOrderBy(ProcurementCardHolderLoad.class, new HashMap(), "creditCardNumber", true);
    ...
    private boolean excludeRecord (Date cancelDate) {
        ...
        if (ObjectUtils.isNotNull(cancelDate)) {
            ...
            if (goLiveDateTS.after(cancelDateTS) && dateTimeService.dateDiff(cancelDateTS, goLiveDateTS, false) > 240) {
                return true;
            }  
            else {
                return false;
