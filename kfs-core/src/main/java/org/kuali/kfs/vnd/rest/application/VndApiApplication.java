 ...
 */
package org.kuali.kfs.vnd.rest.application;
...
@ApplicationPath("/vnd/api/v1")
public class VndApiApplication extends BaseApiApplication {

    public VndApiApplication() {
        super();
        if (SpringContext.getBean(ConfigurationService.class).getPropertyValueAsBoolean("apis.enabled")) {
            singletons.add(new BusinessObjectApiResource("vnd"));
        }
    }
}
