 ...
 */
package org.kuali.kfs.fp.rest.application;
...
@ApplicationPath("/fp/api/v1")
public class FpApiApplication extends BaseApiApplication {

    public FpApiApplication() {
        super();
        if (SpringContext.getBean(ConfigurationService.class).getPropertyValueAsBoolean("apis.enabled")) {
            singletons.add(new BusinessObjectApiResource("fp"));
        }
    }
}
