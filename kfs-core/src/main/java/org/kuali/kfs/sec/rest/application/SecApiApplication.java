 ...
 */
package org.kuali.kfs.sec.rest.application;
...
@ApplicationPath("/sec/api/v1")
public class SecApiApplication extends BaseApiApplication {

    public SecApiApplication() {
        super();
        if (SpringContext.getBean(ConfigurationService.class).getPropertyValueAsBoolean("apis.enabled")) {
            singletons.add(new BusinessObjectApiResource("sec"));
        }
    }
}