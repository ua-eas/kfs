...
/**
 * Suspension Category that checks to see if any Alternate addresses are completed if they exist.
 */
    ...
    public boolean shouldSuspend(ContractsGrantsInvoiceDocument contractsGrantsInvoiceDocument) {
        for (InvoiceAddressDetail addressDetail : contractsGrantsInvoiceDocument.getInvoiceAddressDetails()) {
            if (StringUtils.equals(ArConstants.AGENCY_ALTERNATE_ADDRESSES_TYPE_CODE, addressDetail.getCustomerAddressTypeCode())) {
                ...
                if (!isCustomerAddressComplete(addressDetail.getCustomerAddress())) {
                    return true;
                }
            }
        }
