...
public class PurApLineAction extends CabActionBase {
    ...
    protected void createPurApLineSession(Integer purchaseOrderIdentifier) {
        GlobalVariables.getUserSession().addObject(CamsConstants.CAB_PURAP_SESSION.concat(purchaseOrderIdentifier.toString()), new PurApLineSession());
    }

    protected void clearPurApLineSession(Integer purchaseOrderIdentifier) {
        if (purchaseOrderIdentifier != null) {
            GlobalVariables.getUserSession().removeObject(CamsConstants.CAB_PURAP_SESSION.concat(purchaseOrderIdentifier.toString()));
        }
    }
    ...
    protected void buildPurApDocList(PurApLineForm purApLineForm) {
        ...
        cols.put(CamsPropertyConstants.PurchasingAccountsPayableDocument.PURCHASE_ORDER_IDENTIFIER, purApLineForm.getPurchaseOrderIdentifier());
        Collection<PurchasingAccountsPayableDocument> purApDocs = SpringContext.getBean(BusinessObjectService.class).findMatchingOrderBy(PurchasingAccountsPayableDocument.class, cols, CamsPropertyConstants.PurchasingAccountsPayableDocument.DOCUMENT_NUMBER, true);

        if (purApDocs == null || purApDocs.isEmpty()) {
            GlobalVariables.getMessageMap().putError(KFSConstants.GLOBAL_ERRORS, CamsKeyConstants.ERROR_PO_ID_INVALID, purApLineForm.getPurchaseOrderIdentifier().toString());
        } else {
    ...
    public ActionForward split(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String errorPath = CamsPropertyConstants.PurApLineForm.PURAP_DOCS + KFSConstants.SQUARE_BRACKET_LEFT + purApLineForm.getActionPurApDocIndex() + KFSConstants.SQUARE_BRACKET_RIGHT + "." + CamsPropertyConstants.PurchasingAccountsPayableDocument.PURCHASEING_ACCOUNTS_PAYABLE_ITEM_ASSETS + KFSConstants.SQUARE_BRACKET_LEFT + purApLineForm.getActionItemAssetIndex() + KFSConstants.SQUARE_BRACKET_RIGHT;
    ...
    private PurApLineSession retrievePurApLineSession(PurApLineForm purApForm) {
        PurApLineSession purApLineSession = (PurApLineSession) GlobalVariables.getUserSession().retrieveObject(CamsConstants.CAB_PURAP_SESSION.concat(purApForm.getPurchaseOrderIdentifier().toString()));
        if (purApLineSession == null) {
            purApLineSession = new PurApLineSession();
            GlobalVariables.getUserSession().addObject(CamsConstants.CAB_PURAP_SESSION.concat(purApForm.getPurchaseOrderIdentifier().toString()), purApLineSession);
        }
    ...
    protected void checkSplitQty(PurchasingAccountsPayableItemAsset itemAsset, String errorPath) {
        ...
        if (splitQty == null) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurchasingAccountsPayableItemAsset.SPLIT_QTY, CamsKeyConstants.ERROR_SPLIT_QTY_REQUIRED);
        } else if (splitQty.isLessEqual(KualiDecimal.ZERO) || splitQty.isGreaterEqual(oldQty)) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurchasingAccountsPayableItemAsset.SPLIT_QTY, CamsKeyConstants.ERROR_SPLIT_QTY_INVALID, maxAllowQty.toString());
        }
        return;
    }
    ...
    public ActionForward merge(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (question != null) {
            Object buttonClicked = request.getParameter(KRADConstants.QUESTION_CLICKED_BUTTON);
            if (CamsConstants.TRADE_IN_INDICATOR_QUESTION.equals(question) && ConfirmationQuestion.YES.equals(buttonClicked)) {
                if (purApLineService.mergeLinesHasDifferentObjectSubTypes(mergeLines)) {
                    ...
                    return this.performQuestionWithoutInput(mapping, form, request, response, CamsConstants.PAYMENT_DIFFERENT_OBJECT_SUB_TYPE_CONFIRMATION_QUESTION, warningMessage, KRADConstants.CONFIRMATION_QUESTION, CamsConstants.Actions.MERGE, "");
                } else {
                    performMerge(purApForm, mergeLines, isMergeAll);
                }
            } else if (CamsConstants.PAYMENT_DIFFERENT_OBJECT_SUB_TYPE_CONFIRMATION_QUESTION.equals(question) && ConfirmationQuestion.YES.equals(buttonClicked)) {
                performMerge(purApForm, mergeLines, isMergeAll);
            }
        ...
        if (GlobalVariables.getMessageMap().hasNoErrors()) {
            ...
            if (tradeInIndicatorInSelectedLines && !tradeInAllowanceInAllLines) {
                return this.performQuestionWithoutInput(mapping, form, request, response, CamsConstants.TRADE_IN_INDICATOR_QUESTION, SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(CamsKeyConstants.QUESTION_TRADE_IN_INDICATOR_EXISTING), KRADConstants.CONFIRMATION_QUESTION, CamsConstants.Actions.MERGE, "");
    ...
    protected String generateObjectSubTypeQuestion() {
        ...
        String warningMessage = kualiConfiguration.getPropertyValueAsString(CamsKeyConstants.QUESTION_DIFFERENT_OBJECT_SUB_TYPES) + " " + CamsConstants.Parameters.OBJECT_SUB_TYPE_GROUPS + " " + parameterDetail + ". " + continueQuestion;
    ...
    protected void checkPreTagValidForMerge(List<PurchasingAccountsPayableItemAsset> mergeLines, Integer purchaseOrderIdentifier) {
        ...
        if (!validNumberMap.isEmpty() && validNumberMap.size() > 1 && purApLineService.isMultipleTagExisting(purchaseOrderIdentifier, validNumberMap.keySet())) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.PURAP_DOCS, CamsKeyConstants.ERROR_MERGE_WITH_PRETAGGING);
        }
    }
    ...
    protected void checkMergeAllValid(boolean tradeInAllowanceInAllLines, boolean tradeInIndicatorInSelectedLines) {
        if (tradeInAllowanceInAllLines && tradeInIndicatorInSelectedLines) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.PURAP_DOCS, CamsKeyConstants.ERROR_TRADE_IN_PENDING);
        }
    }
    ...
    protected void checkMergeLinesValid(List<PurchasingAccountsPayableItemAsset> mergeLines, boolean tradeInAllowanceInAllLines, boolean tradeInIndicatorInSelectedLines) {
        if (mergeLines.size() <= 1) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.PURAP_DOCS, CamsKeyConstants.ERROR_MERGE_LINE_SELECTED);
        } else {
            ...
            if (purApLineService.isAdditionalChargePending(mergeLines)) {
                GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.PURAP_DOCS, CamsKeyConstants.ERROR_ADDL_CHARGE_PENDING);
            }
    ...
    protected void checkMergeRequiredFields(PurApLineForm purApForm) {
        if (purApForm.getMergeQty() == null) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.MERGE_QTY, CamsKeyConstants.ERROR_MERGE_QTY_EMPTY);
        }

        if (StringUtils.isBlank(purApForm.getMergeDesc())) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.MERGE_DESC, CamsKeyConstants.ERROR_MERGE_DESCRIPTION_EMPTY);
        }
    }
        ...
        if (question != null) {
            ...
            if ((CamsConstants.TRADE_IN_INDICATOR_QUESTION.equals(question)) && ConfirmationQuestion.YES.equals(buttonClicked)) {
                if (purApLineService.allocateLinesHasDifferentObjectSubTypes(allocateTargetLines, allocateSourceLine)) {
                    // check if objectSubTypes are different and bring the obj sub types warning message
                    String warningMessage = generateObjectSubTypeQuestion();
                    return this.performQuestionWithoutInput(mapping, form, request, response, CamsConstants.PAYMENT_DIFFERENT_OBJECT_SUB_TYPE_CONFIRMATION_QUESTION, warningMessage, KRADConstants.CONFIRMATION_QUESTION, CamsConstants.Actions.ALLOCATE, "");
                } else {
    ...
    protected void performAllocate(PurApLineForm purApForm, PurchasingAccountsPayableItemAsset allocateSourceLine, List<PurchasingAccountsPayableItemAsset> allocateTargetLines) {
        ...
        if (!purApLineService.processAllocate(allocateSourceLine, allocateTargetLines, purApLineSession.getActionsTakenHistory(), purApForm.getPurApDocs(), false)) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.PURAP_DOCS, CamsKeyConstants.ERROR_ALLOCATE_NO_TARGET_ACCOUNT);
        } else {
    ...
    protected void validateAllocateAction(PurchasingAccountsPayableItemAsset allocateSourceLine, List<PurchasingAccountsPayableItemAsset> allocateTargetLines, boolean targetLineHasTradeIn, boolean hasTradeInAllowance, List<PurchasingAccountsPayableDocument> purApDocs) {
        ...
        if (allocateTargetLines.isEmpty()) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.PURAP_DOCS, CamsKeyConstants.ERROR_ALLOCATE_NO_LINE_SELECTED);
        }
        ...
        if (question != null) {
            ...
            if ((CamsConstants.TRADE_IN_INDICATOR_QUESTION.equals(question)) && ConfirmationQuestion.YES.equals(buttonClicked)) {
                ...
                if (isSettingAssetsInPurAp(selectedLine)) {
                    return this.performQuestionWithoutInput(mapping, form, request, response, CamsConstants.SKIP_ASSET_NUMBERS_TO_ASSET_GLOBAL_QUESTION, SpringContext.getBean(ConfigurationService.class).getPropertyValueAsString(CamsKeyConstants.QUESTION_SKIP_ASSET_NUMBERS_TO_ASSET_GLOBAL), KRADConstants.CONFIRMATION_QUESTION, CamsConstants.Actions.CREATE_ASSET, "");
                } else {
                    return createAssetGlobalDocument(mapping, purApForm, selectedLine);
                }
            } else if (CamsConstants.SKIP_ASSET_NUMBERS_TO_ASSET_GLOBAL_QUESTION.equals(question) && ConfirmationQuestion.YES.equals(buttonClicked)) {
                return createAssetGlobalDocument(mapping, purApForm, selectedLine);
            }
            return mapping.findForward(KFSConstants.MAPPING_BASIC);
        }
    ...
    protected void validateCreateAssetAction(PurchasingAccountsPayableItemAsset selectedLine) {
        ...
        if (quantity.isGreaterThan(integerOne) && quantity.mod(integerOne).isNonZero()) {
            GlobalVariables.getMessageMap().putError(CamsPropertyConstants.PurApLineForm.PURAP_DOCS, CamsKeyConstants.ERROR_FRACTIONAL_QUANTITY);
        }
