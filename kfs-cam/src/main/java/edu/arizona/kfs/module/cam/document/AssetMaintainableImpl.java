...
public class AssetMaintainableImpl extends org.kuali.kfs.module.cam.document.AssetMaintainableImpl {
    ...
    public void setGenerateDefaultValues(String docTypeName) {
        ...
        if (getBusinessObject().getObjectId() != null) {
            ...
            if (notes.isEmpty()) {
                Note newBONote = getNewBoNoteForAdding(KFSConstants.CreateAndUpdateNotePrefixes.ADD);
                notes.add(newBONote);
            }
        }
    }
    ...
    private void setAssetCreateAndUpdateNote(List<Note> notes, String prefix) {
        ...
        if (prefix.equals(KFSConstants.CreateAndUpdateNotePrefixes.CHANGE)) {
            ...
            if (!notes.isEmpty()) {
                ...
                if(searchNotesForDocument(getDocumentNumber(), notes)) {
        ...
        if (shouldAddNote) {
            Note newBONote = getNewBoNoteForAdding(prefix);
            notes.add(newBONote);
        }
    }
    
    private boolean searchNotesForDocument(String documentId, List<Note> notes) {
        for (Note note: notes) {
